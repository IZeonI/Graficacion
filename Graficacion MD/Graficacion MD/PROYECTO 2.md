## El objetivo de este proyecto es crear un mundo 3D con diferentes modelos dentro de el y poder explorarlo mediante el uso de flujo óptico
...
##### Estas variables son las que definen donde va a mirar la cámara y a que velocidad se va a mover; look_x es agregado para poder fijar la cámara en una pos_x diferente durante la ejecución y poder volver a mirar al centro del mundo mas tarde
~~~
pos_x = 0
pos_y = 3
pos_z = 10
...
##### Se crea un arreglo que mas tarde se vuelve un diccionario, para poder almacenar las imágenes que serán usadas como texturas junto a un nombre
~~~
textures = {}
~~~


## Importamos las imágenes a usar para la interfaz
##### Al igual que en el proyecto 1 se usaran las mismas flechas para esta interfaz
~~~
...
## Función para ajustar las flechas
~~~
def ajustar(imagen):
    return cv.resize(imagen, (80,80))
~~~

## Creamos un diccionario para las áreas de las flechas
~~~
ubicaciones_flechas = [
...
## Función para verificar si el punto de flujo óptico está dentro de un área de las flechas
~~~
...
##### Misma situación que en el proyecto 1, solo que aquí se aplica para mover a la cámara
~~~
...
~~~         
## Función para dibujar los rectángulos y mostrar las flechas y círculos con los que el usuario va a interactuar
##### Misma interfaz que en el proyecto 1, pero con 2 flechas menos 
~~~
...
~~~
![interfaz proyecto 2](interfaz_proyecto2.png)


## Función para cargar texturas
##### Con esta función se ingresa la ruta de la imagen a cargar como textura y se regresa el id con el que se va a identificar esa textura
~~~
def load_texture(image_path):
    ...
    return texture_id
~~~
## Función para inicializar los métodos de OpenGL y donde guardamos el diccionario de texturas
~~~
def init():
...
~~~
##### Aquí dentro del diccionario textures usando la función load_texture se almacena la imagen cargada como textura que será usada para los modelos mas tarde
~~~
...
## Función para dibujar el suelo del mundo
##### Se especifica el tamaño del cuadrado que será el suelo del mundo y se le asigna una textura
~~~
...
## Función para dibujar cuadrados frontales
##### Se especifica el punto donde inicia el cuadrado, así como su ancho y alto, además se le asigna una textura
~~~
...
## Función para dibujar un cuadrado lateral
##### Mismo caso que el anterior solo que ahora lo dibuja lateralmente
~~~
...
## Función para dibujar un cuadrado viendo hacia arriba
##### Mismo caso que los 2 anteriores pero ahora mirando hacia arriba
~~~
...
##### Usando las 3 funciones anteriores se dibuja un cubo, al igual que antes la función recibe el punto inicial, el ancho y alto, pero ahora recibe también el largo o profundidad del cubo, además que recibe un diccionario con las texturas correspondiente a cada cara del cubo
~~~
...
## Modelo 1 (Árbol)
##### En este modelo aun no definia la funcion para dibujar cubos, por lo cual este modelo en particular esta hecho con las funciones para dibujar cuadrados solamente
~~~
def draw_hoja_arbol():
...
## Modelo 2 (Cerdo)
##### A partir de aqui ya se habia definido la funcion para dibujar cubos por lo cual de este modelo en adelante dentro de la funcion se definen diferentes diccionarios para las texturas de cada cubo necesario para el modelo
~~~
def draw_pig():
...
~~~
![Fence_corner1](fence_corner1.png)
...
~~~
![Fence_corner2](fence_corner2.png)
...
~~~
![Fence_corner3](fence_corner3.png)
...
~~~
![Fence_corner4](fence_corner4.png)
...
def draw_herobrine():
    ...
    draw_cubo(1.02, 1.5, 0, 0.45, 1.5, 0.5, texturas_steve_right_arm)  
~~~
![Herobrine](herobrine.png)


## Posiciones inicial de las vacas
##### Se define un diccionario que servirá para poder hacer una pequeña animación de movimiento de las vacas, en este arreglo se guarda la posición de la vaca y su velocidad de movimiento en X y en Z
~~~
cows_data = [
...
## Función para mover a las vacas en el corral
##### Similar a lo visto en clase, en esta función se define un área de la cual no pueden salir y mediante un ciclo se actualiza el movimiento de las vacas, en caso de que lleguen a uno de los limites se invierte la dirección en ese eje, simulando un "rebote" dentro del corral
~~~
...
## Función para dibujar el entorno con todos los modelos previamente definidos
##### En esta función se definen arreglos para cada modelo con los puntos en los cuales se van a dibujar dicho modelo, además de inicializar la cámara y dibujar el suelo
~~~
def draw_entorno():
...
##### Se agrego esta función para comodidad del programador, ya que permite un movimiento mas sencillo que el que permite el flujo óptico
~~~
